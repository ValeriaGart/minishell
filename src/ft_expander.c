...
/*   Updated: 2024/01/30 13:46:30 by vharkush         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_expand_global(int *i, char *new, t_data *data)
{
	char	*get_var;

	get_var = ft_itoa(data->exit_code);
	if (!get_var)
		return (ft_free_new(new));
	new = ft_strjoin_free(new, get_var);
	*i = *i + 1;
...
char	*process_character(char *str, char *new, int *i, t_data *data)
{
		...
		&& str[*i + 1] && (str[*i + 1] == '$' || str[*i + 1] == '\\'))
	{
		...
		new = ft_expand_global(i, new, data);
	else
